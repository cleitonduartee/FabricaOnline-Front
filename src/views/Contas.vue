<template>
  <div>
    ...
    <!-- lista de contas -->
    <v-data-table :headers="cabecalhoContas" :items="contas" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>Contas Cadastrados</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="abrirFormulario" v-if="!mostrarForm">Novo Conta</v-btn>
        </v-toolbar>
      </template>
      ...
      <template v-slot:item.acoes="{ item }">
        <v-icon small class="mr-2" @click="detalhes(item)" color="blue">mdi-magnify</v-icon>
        <v-icon
          small
          @click="excluirConfirm(item)"
          color="red"
     ...
     <v-dialog
      v-model="dialogConfirmaExclusao"
      max-width="455"
      persistent
    >
      <v-card>
        <v-card-title class="headline">Deseja Realmente excluir essa conta?</v-card-title>
        ...
        <v-card-text>
          ...
          <v-col cols="12" sm="4">
            <v-text-field
              ...
              type="number"
              disabled
              :value="contaAtualDetalhes.valor?contaAtualDetalhes.valor.toFixed(2):contaAtualDetalhes.valor"              
              hide-details>
            </v-text-field>
          </v-col>
        </v-row>   
